...
def analyze_    # Let's also check what the actual first experiment configs look like
    ...
    for i, config in enumerate(state['experiment_configs'][:3]):
        ...
        print()   checkpoint_file = 'extensive_results/checkpoints/checkpoint_20250611_161729_764.yaml'
    ...
    for i, config in enumerate(state['experiment_configs'][:3]):
        ...
        attack = config.get('attack', 'none')
        dataset = config.get('dataset', 'unknown')
        attack_params = config.get('attack_params', {})        if attack == 'none' or not attack_params:
            exp_id = f"{strategy}_{attack}_{dataset}"
        else:
            params_str = "_".join([f"{k}{v}" for k, v in sorted(attack_params.items())])
            exp_id = f"{strategy}_{attack}_{params_str}_{dataset}"
